...
if __name__ == '__main__':
    ...
    parser.add_argument('--proposal_dir', type=str, default='/home/baikai/Desktop/AliComp/datasets/PreRoundData/proposals/',
                        help='Path to the directory containing json files with proposals, optical flow warping, reid vectors')
    parser.add_argument('--output_dir', type=str, default='/home/baikai/Desktop/AliComp/datasets/PreRoundData',
    ...
    image_dir = proposal_dir.replace('proposals', 'JPEGImages')
    ...
    for name in names:
        ...
        all_props, img_size, t_0 = get_proposals_info(prop_dir=proposal_dir, name=name,
                                                      image_dir=image_dir, arch='resnet101')
        ...
        for t, props in enumerate(all_props):
            ...
            save_with_pascal_colormap(os.path.join(output_dir, 'results', name + '/' +
                                                   str(t + t_0).zfill(5) + '.png'), png)

        print('DONE in %d seconds' % int(time.time() - tickle))
